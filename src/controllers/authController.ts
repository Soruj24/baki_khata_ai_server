...
import {
  ...
  type IUser,
  type IAvatar,
} from "../models/interfaces/IUser.ts";
...
import { findUser } from "../services/userServices.ts";
...
import cloudinary, { uploadToCloudinary } from "../config/cloudinary.ts";
import {
  getClientIP,
  sanitizeUser,
  checkAccountLockout,
...
const ua = (r: Request): Record<string, string> => {
  const v = r.get("User-Agent");
  return v ? { userAgent: v } : {};
};

const pickDefined = <T extends Record<string, any>>(obj: T): { [K in keyof T]-?: T[K] } => {
  const result = {} as any;
...
const handleCreateUser = asyncHandler(
  async (
    ...
    const newUserData = {
      ...
      ...pickDefined({ detectedCountry: req.headers["cf-ipcountry"] as string }),
      metadata: {
        ...ua(req),
        ipAddress: userIP,
        signupFlow: "standard",
    ...
    const user = await User.create(newUserData as any);
  ...
  async (
    ...
    if (!user) {
      ...
      if (user) {
        ...
        // Create new user
        const baseUsername: string = username || email?.split("@")[0] || "user";
...
const handleChangedPassword = asyncHandler(
  async (
    req: Request<Record<string, string>, {}, PasswordChangeBody>,
    res: Response,
    next: NextFunction,
  ) => {
...
const handleDeleteAccount = asyncHandler(
  async (
    ...
    (user as any).status = UserStatus.DELETED;
    ...
    try {
      ...
      if (emailResult.success) {
        ...
        return successResponse(res, {
          statusCode: 200,
          message:
            "Email updated successfully, but we failed to send the verification email. Please try resending it from your profile.",
          payload: {
            email: newEmail,
            emailVerified: false,
          },
      ...
      console.error(
        "Unexpected error sending verification email for email update:",
        error,
      );
    }
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (role === "super_admin" && req.user!.role !== "super_admin") {
        return next(
          createError(403, "Only super admin can assign super_admin role"),
        );
      }
      ...
      if (user.role === "super_admin" && req.user!.role !== "super_admin") {
        return next(
          createError(403, "Only super admin can modify other super admins"),
        );
      }
  ...
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!emailResult.success) {
        ...
        return next(
          createError(500, "Failed to send email: " + emailResult.error),
        );
      }
...
const handleUpdateUser = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (userId === adminId.toString()) {
        return next(
          createError(400, "Use profile update endpoint for your own account"),
        );
      }
...
const handleAdminCreateUser = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (role === "super_admin" && req.user!.role !== "super_admin") {
        return next(
          createError(403, "Only super admin can create super admin accounts"),
        );
      }
      ...
      console.log(
        `Admin created user: ${user.email} with verification token: ${emailVerificationToken}`,
      );
...
const handleAdminRevokeAllSessions = asyncHandler(
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      // Revoke all sessions
      await Session.updateMany(
        { userId: userId as any, revokedAt: { $exists: false } as any },
        {
          revokedAt: new Date(),
          revokedBy: adminId,
