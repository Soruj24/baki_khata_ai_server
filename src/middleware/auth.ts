...
import { jwtAccessKey } from "../secret.ts";
import {
  createJSONWebToken,
  verifyJSONWebToken,
} from "../helper/jsonwebtoken.ts";
import User from "../models/schemas/User.ts";
import { UserRole, Permission } from "../models/interfaces/IUser.ts";
  ...
  try {
    ...
    try {
      ...
      if (tokenExp && tokenExp - now < 1800) {
        const newToken = createJSONWebToken(
  ...
  return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (!allowedRoles.includes(req.user.role)) {
        throw createHttpError(
          403,
          `Access denied: ${req.user.role} role does not have permission to access this resource`,
        );
      }
