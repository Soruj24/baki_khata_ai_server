  ...
  },
  {
    ...
    toJSON: {
      ...
      transform: function(doc, ret) {
        ...
        delete json.stripePaymentMethodId;
        delete json.fingerprint;
...
paymentMethodSchema.pre('save', function () {
  if (this.expiryMonth && this.expiryYear) {
    ...
    if (expiryDate < now) {
      ...
      // expiryMonth error
      error.errors.expiryMonth = new mongoose.Error.ValidatorError({ message: 'Card has expired' });
      ...
      // expiryYear error
      error.errors.expiryYear = new mongoose.Error.ValidatorError({ message: 'Card has expired' });
