...
import type { IUserDoc } from "../types/UserTypes.ts";
import type { ILoginHistory } from "../interfaces/IUser.ts";
import { UserStatus, UserRole, ProfileVisibility, Permission } from "../interfaces/IUser.ts";
import { USER_CONSTANTS } from "../constants/UserConstants.ts";
import { generateSecureToken, hashToken, sanitizeUserAgent, getDeviceInfo, calculateAge } from "../utils/UserUtils.ts";
...
export const applyUserMethods = (schema: Schema<IUserDoc>) => {
  ...
  schema.methods.incrementLoginAttempts = async function (): Promise<IUserDoc> {
    ...
    if (this.loginAttempts >= USER_CONSTANTS.LOCKOUT.MAX_ATTEMPTS) {
      ...
      this.lockoutUntil = new Date(Date.now() + (delayMinutes ?? 0) * 60 * 1000);
